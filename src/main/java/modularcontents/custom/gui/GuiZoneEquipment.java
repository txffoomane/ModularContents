...
public class GuiZoneEquipment extends GuiScreen {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (isInsideMap(mouseX, mouseY)) {
            dragging = true;
            startWorldX = worldXFromMouse(mouseX);
            startWorldZ = worldZFromMouse(mouseY);
            selMinX = startWorldX;
            selMaxX = startWorldX;
            selMinZ = startWorldZ;
            selMaxZ = startWorldZ;
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        if (button.id == 0) {
            ...
            ModularcontentsMod.PACKET_HANDLER.sendToServer(
                    new PacketZoneLoot(selMinX, selMinZ, selMaxX, selMaxZ, false, selections));
        } else if (button.id == 1) {
            ModularcontentsMod.PACKET_HANDLER.sendToServer(
                    new PacketZoneLoot(selMinX, selMinZ, selMaxX, selMaxZ, true, new ArrayList<>()));
        }
    }
